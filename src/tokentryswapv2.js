...
async function dumpTokens() {
    ...
    const tokenTools = new ethers.Contract(
        TOKEN_TOOLS_ADDRESS,
        TOKEN_TOOLS_ABI,
        account
    );
    ...
    try {
        console.log(
            'DEBUG: Restricting to tokens that need retrying... Overriding valid tokens.'
        );
        filteredTokenInfo = JSON.parse(
            fs.readFileSync('data/dump_retry_tokens.json', 'utf8')
        );
        console.log(
            'DEBUG: Retry token count: ',
            Object.keys(filteredTokenInfo).length
        );
    } catch (err) {
    ...
    const tokenChunksSwapTest = tokenListFull.reduce(
    ...
    try {
        ...
        console.log(
            'Found valid token file. Read ',
            Object.keys(readValid).length,
            ' valid tokens.'
        );
    ...
    try {
        ...
        const readInvalid = JSON.parse(
            fs.readFileSync('data/dump_invalid_tokens.json', 'utf8')
        );
        console.log(
            'Found invalid tokens file. Read ',
            Object.keys(readInvalid).length,
            ' invalid tokens.'
        );
    ...
    fs.writeFileSync(
        'data/dump_retry_tokens.json',
        JSON.stringify(retryTokens),
        'utf8'
    );
    console.log(
        "Token data 'retry' saved to file. Warning: need to retry manually."
    );
    // Invalid tokens
    if (chunkSizeSwapTest == 1) {
        // If we are at the finest granularity, save invalid tokens to file
        console.log(
            'Finest granularity reached (CHUNK_SIZE == 1). Retry tokens are conbsidered invalid.'
        );
        Object.assign(invalidTokens, retryTokens);
        console.log(
            "The 'retry' tokens fail in a way not handled by the TokenTools contract."
        );
    }
    console.log('Failed tokens: ', Object.keys(invalidTokens).length);
    fs.writeFileSync(
        'data/dump_invalid_tokens.json',
