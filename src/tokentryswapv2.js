...
async function dumpTokens() {
    ...
    let retryTokens = {};
    try {
        // Read retryTokens from file if it exists
        const readRetry = JSON.parse(fs.readFileSync('data/dump_retry_tokens.json', 'utf8'));
        console.log("Found retry tokens file. Read ", Object.keys(readRetry).length, " retry tokens.");
        retryTokens = readRetry;
    } catch (err) {
        console.log("No retry token file found.");
    }
    let chunkCount = 0;
    for (const tokenChunk of tokenChunksSwapTest) {
        ...
        try {
            ...
            swapResults.forEach((result, index) => {
                if (result) {
                    validTokens[tokenChunk[index]] = filteredTokenInfo[tokenChunk[index]];
                } else {
                    invalidTokens[tokenChunk[index]] = filteredTokenInfo[tokenChunk[index]];
                }
            });
        } catch (err) {
    ...
    console.log("Token data retry saved to file. Warning: need to retry manually (move into failed_tokens.json ?)");
}
