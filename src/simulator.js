...
function exactTokensOut(amountIn, pool, zfo) {
    ...
    if (pool.version == 2) {
        ...
        amountOut =
            (inAfterFees * reserveOut) / (10000n * reserveIn + inAfterFees);
        ...
        try {
            if (zfo) {
                ...
                let sqrtPricePrimeX96 =
                    (liquidity * sqrtPriceX96 * q96 * BigInt(10000)) /
                    (BigInt(10000) * q96 * liquidity +
                        inAfterFees * sqrtPriceX96);
                ...
                let sqrtPricePrimeX96 =
                    sqrtPriceX96 +
                    (q96 * inAfterFees) / (liquidity * BigInt(10000));

                // Find the sqrtPrice to the tick right above the current sqrtPrice.
                let tickUpper =
                    (1 + Math.floor(currentTick / tickSpacing)) * tickSpacing;
...
async function simulatePendingTransactions(
    pendingTxArray,
    httpRpcUrl,
    blockNumber
) {
    ...
    const typedTxArray = pendingTxArray.map((txData) =>
        TransactionFactory.fromTxData(
            {
                ...txData,
                gasPrice: txData['gasPrice'].toHexString(),
                maxPriorityFeePerGas:
                    txData['maxPriorityFeePerGas']?.toHexString(),
    ...
    for (let txResult of txResultArray) {
        if (txResult.receipt.logs.length > 0) {
            logs = logs.concat(
                txResult.receipt.logs.map((log) => {
                    return {
                        address: '0x' + Buffer.from(log[0]).toString('hex'),
                        topics: log[1].map(
                            (topicUnit8Array) =>
                                '0x' +
                                Buffer.from(topicUnit8Array).toString('hex')
                        ),
