...
async function dumpTokens() {
    ...
    try {
        let failedChunks = JSON.parse(
            fs.readFileSync('data/dump_failed_chunks.json', 'utf8')
        );
        if (failedChunks.length > 0) {
            console.log(
                'Found failed',
                failedChunks.length,
                'chunks. Overriding the list with the failed chunks.'
            );
    ...
    for (const tokenChunk of tokenChunks) {
        ...
        try {
            ...
            console.log(
                'Error getting token info. Index: ',
                i,
                ' Error: ',
                err
            );
    ...
    fs.writeFileSync(
        'data/dump_failed_chunks.json',
        JSON.stringify(failedChunks),
        'utf8'
    );
    console.log(
        'Failed chunks saved to file. They will be retried on the next run.'
    );
    ...
    try {
        badTokens = JSON.parse(
            fs.readFileSync('data/dump_bad_token_info.json', 'utf8')
        );
        console.log(
            'Found bad tokens file. Read ',
            Object.keys(badTokens).length,
            ' bad tokens.'
        );
    } catch (err) {
    ...
    if (CHUNK_SIZE == 1) {
        ...
        console.log(
            'Finest granularity reached. The failed tokens are toxic in a way that is not handled by the TokenTools contract.'
        );
        ...
        console.log(
            'Failed tokens flagged as bad tokens: ',
            flatFailedChunks.length
        );
    }
    // Save bad tokens to a file
    console.log('Total tokens with bad info: ', Object.keys(badTokens).length);
    fs.writeFileSync(
        'data/dump_bad_token_info.json',
        JSON.stringify(badTokens),
        'utf8'
    );
    // Save filtered token info to a file
    console.log(
        'Filtered token info count: ',
        Object.keys(filteredTokenInfo).length
    );
    fs.writeFileSync(
        'data/dump_token_info.json',
        JSON.stringify(filteredTokenInfo),
        'utf8'
    );
}
