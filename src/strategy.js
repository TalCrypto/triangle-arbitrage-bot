...
async function main() {
    ...
    for (let pool of Object.values(Object.assign(pools_v2, pools_v3))) {
        ...
        if (approvedTokenArray.includes(pool.token0) && approvedTokenArray.includes(pool.token1)) {
    ...
    wss.on('block', async (blockNumber) => {
        ...
        try {
            ...
            let profitablePaths = [];
            logger.info(
                `Evaluating ${touchedPaths.length} touched paths. Block #${blockNumber}`
            );
            ...
            promises = promises.concat(
                providers.map((pvdr) =>
                    pvdr.send('eth_sendRawTransaction', txObject)
                )
            );
            promises.forEach((promise, index) => {
                promise
                    ...
                    .catch((e) => {
                        ...
                        logger.error(
                            `Error while sending to ${HTTP_ENDPOINTS[index]}: ${e}`
                        );
                    });
            });
            ...
            logger.info(
                ...
                } s after block #${blockNumber}`
            );
            await Promise.all(promises);
            logger.info(
                `Successfully received by ${successCount} endpoints. E2E ${
                    (Date.now() - start) / 1000
                } s. Tx hash ${await promises[0]} Block #${blockNumber}`
            );
            lastTxCount++;
        } catch (e) {
