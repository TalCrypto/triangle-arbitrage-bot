  ...
  wsProvider.on("block", async (blockNumber) => {
    ...
    try {
      ...
      const tempWithFilter = await Promise.all(
        pendingTxArray.map(async (e) => {
          ...
          return {
            value: e,
            filter:
              (receipt === null ?? false) && blockNumber - e.blockNumber < 3,
          };
        })
      );
      ...
      if (fetchPools.length > 0) {
        ...
        logger.info(
          `${(e - s) / 1000} s - Batch reserves call. Block #${blockNumber}`
        );
      }
      ...
      logger.info(
        `Profitable path: $${path.profitusd} ${
          ...
          Number(path.profitwei) / 10 ** SAFE_TOKENS[path.rootToken].decimals
        } block #${blockNumber}`
      );
      ...
      logger.info(
        `Finished sending. End-to-end delay ${(Date.now() - sblock) / 1000} s`
      );
    } catch (e) {
      ...
      logger.info(
        `=== End of block #${blockNumber} (took ${blockElapsed / 1000} s)`
      );
    }
  });
