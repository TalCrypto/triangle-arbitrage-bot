...
async function findUpdatedPools(provider, blockNumber, pools, tokens) {
    ...
    for (let iface of interfaces) {
        ...
        prom.then((logs) => {
            logger.info(
                `Found ${logs.length} logs for ${Object.values(iface.events)[0].name
...
function displayStats(sessionStart, logger, tokens, dataStore, profitStore) {
    ...
    logger.info(
        `Session duration: ${sessionDuration} seconds (${sessionDuration / 60
    ...
    for (let token in profitStore) {
        ...
        logger.info(
            `${tokens[token].symbol}: ${profit} $${profit * tokens[token].usd
            } (${token})`
        );
    }
    logger.info("========================");
...
function getPoolsFromLogs(logs, pools) {
    ...
    for (let log of logs) {
        ...
        if (syncEvtTopic == log.topics[0]) {
            ...
            touchablePoolAddresses.push(checksumPoolAddress);
        } else if (mintEvtTopic == log.topics[0] || burnEvtTopic == log.topics[0]) {
            touchablePoolAddresses.push(checksumPoolAddress);
        }
    }
...
function isSamePendingArrays(pendingArray1, pendingArray2) {
    ...
    for (let i = 0; i < pendingArray1.length; i++) {
        if (pendingArray1[i].txnData['hash'] !== pendingArray2[i].txnData['hash']) return false;
    }
    return true;
}
