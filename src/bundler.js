...
async function buildLegacyTx(path, tradeContract, tokens, logger, signer, lastTxCount, gasPrice) {
    // Display info about the path. Prepare the parameters
    path.amounts = [path.amountIn.toString()];
    let amountOut = path.amountIn;
    for (let i = 0; i < path.pools.length; i++) {
...
async function buildBackRunTx(path, tradeContract, tokens, logger, signer, lastTxCount, targetTx) {
    // Display info about the path. Prepare the parameters
    path.amounts = [path.amountIn.toString()];
    let amountOut = path.amountIn;
    for (let i = 0; i < path.pools.length; i++) {
