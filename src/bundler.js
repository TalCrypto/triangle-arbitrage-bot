...
async function buildFrontRunTx(
  path,
  tradeContract,
  tokens,
  logger,
...
function getInitialAction(path, tradeContract, tokens, logger) {
  ...
  for (let i = 0; i < path.pools.length; i++) {
    ...
    if (pool.version == 2) {
      logger.info(
        `pool v:${pool.version} a:${pool.address} z:${zfo} tin:${tin} (${
          tokens[tin].symbol
        }) tout:${tout} (${tokens[tout].symbol}) in:${path.amounts[i]} out:${
          path.amounts[i + 1]
        } r0:${pool.extra.reserve0} r1:${pool.extra.reserve1}`
      );
      ...
      logger.info(
        `pool v:${pool.version} a:${pool.address} z:${zfo} tin:${tin} (${
          ...
          path.amounts[i + 1]
        } s:${pool.extra.sqrtPriceX96} l:${pool.extra.liquidity}`
      );
    }
  }
